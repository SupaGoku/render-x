...
- No test runner is wired yet; when adding tests, prefer Vitest + happy-dom (see `.blueprint/product/tech-stack.md`).

---

## Roadmap

- ✅ v1.0.0: Core renderer, hooks, portals, CSS helpers.
- 📚 Documentation site powered by VitePress (planned).

For additional priorities, consult [`./.blueprint/product/roadmap.md`](.blueprint/product/roadmap.md).
...
3. Create feature specs under `.blueprint/specs/` when proposing larger changes.
4. Run `yarn build` before submitting a PR to ensure bundles compile.
5. Follow the TypeScript code style in `.blueprint/standards/code-style/typescript-style.md`.

Issues and PRs are welcome! Please link to relevant specs or create one if the change is product-affecting.
